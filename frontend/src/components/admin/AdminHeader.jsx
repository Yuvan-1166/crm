...
/**
 * AdminHeader - Dashboard header with navigation tabs and user menu
 ...
 */
const TabButton = memo(({ tab, isActive, onClick, mobile = false }) => {
  const Icon = tab.icon;
  
  const baseClasses = "flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all";
  const activeClasses = "bg-sky-500 text-white shadow-md shadow-sky-500/30";
  const inactiveClasses = "bg-gray-100 text-gray-600 hover:bg-gray-200 hover:text-gray-900";
  const mobileClasses = mobile ? "whitespace-nowrap" : "";
